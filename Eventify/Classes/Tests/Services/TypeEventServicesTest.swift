...
class TypeEventServicesTest: NSObject {
    static let shared = TypeEventServicesTest()
    ...
    func getTypesEvent(completionHandler: @escaping (_ types: [TypeObjectTest]?, _ error: String?) -> Void) {
        ...
        socketType.once("get-types") { (data, ack) in
            Helpers.errorHandler(with: data, completionHandler: { (json, error) in
                ...
                guard let types = [TypeObjectTest].from(jsonArray: json) else {
